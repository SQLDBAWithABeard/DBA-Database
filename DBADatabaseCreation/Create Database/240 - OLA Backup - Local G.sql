 /*
 This script will set the backup jobs for the OLA maintenance plan solution to use Local G Drive  - Line 83
 */
 ...
 DECLARE @BackupDirectory nvarchar(max)
  ...
  SELECT @BackupDirectory = 'G:\' /* THIS is the backup directory*/
...
END
  ...
  SET @JobOwner = SUSER_SNAME(0x01)
    ...
    SET @JobName01 = 'DatabaseBackup - SYSTEM_DATABASES - FULL - Local G Drive'
...
EXEC msdb.dbo.sp_delete_job @job_name=@JobName01 , @delete_unused_schedule=1
  ...
  SET @JobName02 = 'DatabaseBackup - USER_DATABASES - DIFF - Local G Drive'
...
EXEC msdb.dbo.sp_delete_job @job_name=@JobName02 , @delete_unused_schedule=1
  ...
  SET @JobName03 = 'DatabaseBackup - USER_DATABASES - FULL - Local G Drive'
...
EXEC msdb.dbo.sp_delete_job @job_name=@JobName03 , @delete_unused_schedule=1
 ...
 SET @JobName04 = 'DatabaseBackup - USER_DATABASES - LOG - Local G Drive'
...
Declare @RestoreCommand nvarchar(max)
 ...
 IF @Version >= 9.002047
  ...
  Declare @RestoreScriptDir nvarchar(max) = @BackupDirectory  + '\' + REPLACE(CAST(SERVERPROPERTY('servername') AS nvarchar),'\','$')   -- Choose restore script location: 'Backup_Dir', 'Error_Log', or custom defined dir, e.g., 'C:\' . Directory must be created first if custom!
